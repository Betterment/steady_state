...
- State predicate methods (e.g. `open?`) are now defined with explicit zero
  arity instead of via `delegate`. When a model using `steady_state` lives
  inside a gem, tapioca captures its predicates in the gem's RBI; the
  splat-arg methods generated by `delegate` conflicted (Sorbet error 4010)
  with the zero-arity predicates in DSL-generated RBIs, causing `tapioca dsl`
  to downgrade the gem RBI to `typed: false`. Calling a predicate with
  arguments (previously silently ignored) now raises `ArgumentError`.
